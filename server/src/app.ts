...
export function createApp(config: AppConfig, dependencies: AppDependencies) {
   ...
   */
  let knownRoutePathsCache: Set<string> | null | undefined;
   ...
   *
   * The manifest is generated by `scripts/prerender.mjs` into the static
   * bundle, so the server never hard-codes a route list that could drift from
   * `src/lib/site-routes.ts`.
   *
   * @returns {Promise<Set<string> | null>} Known route paths, or null when no
   *   manifest is present — in which case route validation is unavailable and
   *   the caller must fall back to serving the shell with a 200.
   */
  async function loadKnownRoutePaths(): Promise<Set<string> | null> {
    if (knownRoutePathsCache !== undefined) {
      return knownRoutePathsCache;
    }
    ...
    try {
      ...
      if (!Array.isArray(parsed.paths) || parsed.paths.length === 0) {
        throw new Error("route-manifest.json has no paths array");
      }
      knownRoutePathsCache = new Set(parsed.paths.filter((p): p is string => typeof p === "string"));
    } catch (error) {
      console.warn(
  ...
  async function serveIndexHtml(request: Request, url: URL): Promise<Response> {
    ...
    const knownPaths = await loadKnownRoutePaths();
    ...
    const isKnownRoute = knownPaths === null || knownPaths.has(routePath);

    const candidates = isKnownRoute
      ? [prerenderedDocumentPath(routePath), "/index.html"]
      : ["/not-found.html", "/index.html"];
