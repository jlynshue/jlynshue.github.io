...
/** Parsed form of `dist/route-manifest.json`, produced by scripts/prerender.mjs. */
interface RouteManifest {
  /** Route path -> static-bundle-relative prerendered document. */
...
export function createApp(config: AppConfig, dependencies: AppDependencies) {
  ...
  /**
   * Memoized route manifest. `undefined` means "not yet read"; `null` means
   * "read and unusable", which is a distinct, sticky state — a missing manifest
  ...
  /**
   * Loads the prerenderer's route manifest, memoized for the process lifetime.
   *
   * The manifest is generated by `scripts/prerender.mjs` into the static bundle
   * and is the single authority for two things: which paths are real routes,
   * and which prerendered document answers each one. The server deliberately
   * does not recompute document paths — the prerenderer parks most documents
   * outside the route namespace so Firebase Hosting cannot intercept them, and
   * duplicating that layout rule here is exactly how the two would drift.
   *
   * @returns {Promise<RouteManifest | null>} The manifest, or null when none is
   *   present — in which case route validation is unavailable and the caller
   *   must fall back to serving the shell with a 200.
   */
  async function loadRouteManifest(): Promise<RouteManifest | null> {
    ...
    try {
      ...
      routeManifestCache = {
        documents: new Map(
          Object.entries(documents as Record<string, unknown>).filter(
      ...
      console.warn(
        `[app] no usable route-manifest.json in ${config.staticDir}; soft-404 protection disabled (${
  ...
  async function serveIndexHtml(request: Request, url: URL): Promise<Response> {
    ...
    const candidates = isKnownRoute
      ...
      : [manifest?.notFoundDocument, "/index.html"];

    for (const candidate of candidates) {
      if (!candidate) {
        continue;
      }
      const htmlResponse = await serveStaticFile(config.staticDir, candidate, request.method);
      if (!htmlResponse) {
        continue;
      }
